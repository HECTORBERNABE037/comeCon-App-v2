...
class DatabaseService {
  ...
  // 2. CREACIÓN DE TABLAS (Solo las necesarias por ahora)
  private async createTables(): Promise<void> {
    if (!this.db) return;
    
    // Tabla de USUARIOS (Espejo de Django + Password para offline)
    ...
    // (Más adelante agregaremos aquí las tablas de Products y Orders)
  }
  ...
  // Guardar o Actualizar Usuario desde la API (Sync)
  async syncUser(apiUser: any, passwordInput: string): Promise<void> {
    if (!this.db) return;
    
    try {
      // Verificamos si ya existe
      const existing = await this.db.getFirstAsync('SELECT id FROM users WHERE email = ?', [apiUser.email]);

      // Mapeo de datos (Asegúrate que los campos coincidan con lo que manda Django)
      const role = apiUser.role || 'cliente';
      const name = apiUser.name || '';
      const nickname = apiUser.nickname || '';
      const phone = apiUser.phone || '';
      const image = apiUser.image || ''; // URL de la imagen

      if (existing) {
        // ACTUALIZAR
        await this.db.runAsync(
          `UPDATE users SET name=?, nickname=?, role=?, password=?, phone=?, image=? WHERE email=?`,
          [name, nickname, role, passwordInput, phone, image, apiUser.email]
        );
      } else {
        // INSERTAR
        await this.db.runAsync(
          `INSERT INTO users (email, password, name, nickname, role, phone, image) VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [apiUser.email, passwordInput, name, nickname, role, phone, image]
        );
      }
  ...
  // Obtener usuario local (Para Login Offline y Persistencia)
