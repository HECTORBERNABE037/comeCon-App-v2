...
export const ProductDetailScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  // Lógica de Precios (Adaptación SQLite)
  // Si existe promotionalPrice, ese es el precio activo. Si no, es el precio base.
  ...
  const handleAddToCart = () => {
    const total = (parseFloat(activePrice) * quantity).toFixed(2);
    Alert.alert(
      'Añadido al carrito',
      `${quantity}x ${platillo.title} - Total: $${total}`
    );
    navigation.goBack(); 
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {/* 3. Selector de Cantidad Flotante */}
        ...
        <View style={styles.infoContainer}>
          ...
          {/* PRECIO (Adaptado para mostrar oferta) */}
          <View style={styles.priceRow}>
             {/* Si hay promo, mostramos el original tachado */}
             {hasPromo && (
                <Text style={styles.oldPriceText}>${platillo.price}</Text>
             )}
             
             {/* Precio Activo (Verde si es oferta, Naranja si es normal) */}
             <Text style={[
               styles.price, 
               hasPromo && { color: '#2E7D32' } // Verde Oferta
             ]}>
               ${activePrice}
             </Text>
          </View>
          ...
          <Text style={styles.descriptionText}>
            {platillo.description || "Sin descripción disponible."}
          </Text>
...
const styles = StyleSheet.create({
  ...
  quantitySelector: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.white, borderRadius: 25,
    paddingHorizontal: 20, paddingVertical: 10, height: 50, elevation: 4,
